...
class InspectorDebuggingCanvas:
    ...
    def render_inspector_text(self):
        ...
        s = pygame.Surface((GameScreen.RealScreenWidth // 4, GameScreen.RealScreenHeight - 20))  # the size of your rect
        s.set_alpha(128)  # alpha level
        s.fill((0, 0, 0))  # this fills the entire surface
        GameScreen.GameScreenRealSurface.blit(s, (10, 10))  # (0,0) are the top-left coordinates

        # the msgs
        msgs = "JNETO PRODUCTIONS: INSPECTOR SYSTEM\n\n" \
               ...
               f"{GameScreen.get_inspector_status()}\n" + \
               f"{InputManager.get_inspector_status()}\n" \
               f"{self.current_scene.get_inspector_status()}\n" \
               f"{self.current_scene.camera.get_inspector_status()}"

        # calls the method that displays text on the dummy screen
        TextRenderOverlaySystem.blit_text(GameScreen.GameScreenRealSurface, GameScreen.RealScreenWidth // 3,
                                          msgs, (30, 30), font, color=pygame.Color("white"))

    def render_game_object_inspector_status(self, index_of_game_obj, color: str):
        font = pygame.font.Font('engine_JNeto_Productions/_engine_resources/fonts/JetBrainsMono-Medium.ttf',
                                self.font_size)  # create a text surface object,
        msgs = f"{self.current_scene.game_object_list[index_of_game_obj].get_inspector_status()}\n"
        # calls the method that displays text on the dummy screen
