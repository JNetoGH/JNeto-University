...
class SceneCamera:
    ...
    def get_inspector_status(self) -> str:
        ...
        if self.__followed_game_object is not None:
            text += f"followed GameObject: {self.__followed_game_object.name}\n"
            text += f"following GameObject off-set: {self.followed_object_offset}\n"
        else:
            text += f"followed GameObject: None\n"

        # RENDERING LAYERS
        text += f"total rendering layers: {len(self.rendering_layers_list)}\n"
        tot_in_layers = 0
        info_about_each_layer = ""
        for render_layer in self.rendering_layers_list:
        ...
        text += f"total game objects in rendering layers: {tot_in_layers}\n" \
